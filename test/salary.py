...
def linkedin_salary_scraper(search_query, location):
    ...
    for listing in listings:
        job_title = listing.find_all("h3", class_="result-card__title").text.strip()
        company = listing.find_all("h4", class_="result-card__subtitle").text.strip()
        location = listing.find_all("span", class_="job-result-card__location").text.strip()

        salary_info = listing.find_all("span", class_="job-search-card__salary-info")
        if salary_info:
...
if __name__ == "__main__":
    search_query = "Manager"  # Change this to your desired job title
